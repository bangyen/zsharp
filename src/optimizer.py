...
class ZSharp(SAM):
    def __init__(self, params, base_optimizer, rho=0.05, percentile=70, **kwargs):
        super().__init__(params, base_optimizer, rho=rho, **kwargs)
        self.percentile = percentile

    @torch.no_grad()
    def first_step(self):
        grads = [p.grad.detach().flatten() for group in self.param_groups for p in group['params'] if p.grad is not None]
